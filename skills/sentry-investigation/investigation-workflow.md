...
**Comprehensive investigation process with business context.**

#### Step 0: Find Related Tickets and Documentation (CRITICAL)

**Before diving into technical details, establish business context:**

```bash
# 1. Extract file path from stack trace
# Example: app/services/payment_processor.rb:42

# 2. Find recent commits affecting that file
gh api repos/owner/repo/commits \
  --jq '.[] | select(.commit.message | contains("payment_processor")) | {sha: .sha[0:7], message: .commit.message, date: .commit.author.date}' \
  | head -5

# 3. Extract ticket IDs from commit messages
...
# 4. Fetch Jira ticket for business context
mcp__atlassian__getJiraIssue({
  cloudId: "employmenthero",
  issueIdOrKey: "PROJ-123"
})

# 5. Scan Jira description for Confluence links
# Pattern: https://[org].atlassian.net/wiki/spaces/[SPACE]/pages/[ID]

# 6. Fetch Confluence pages for implementation guides
...
**What this reveals:**
- **Why was the change made?** (Jira ticket context)
- **What was the intended behavior?** (Acceptance criteria)
- **What are the correct patterns?** (Confluence documentation)
- **Was there a misunderstanding?** (Developer intent vs requirements)

**When to use this step:**
- ✅ Error started after recent deployment/release
- ✅ Stack trace points to recently modified files
- ✅ Need to understand business requirements
- ✅ Want to prevent recurrence through documentation
- ⚠️ Optional if Atlassian MCP unavailable or error is in old code
...
- Frequency (events count, users affected)
...
{
  "id": "12345",
  "title": "StripeAPIError: Card declined",
  "status": "unresolved",
  "count": "47",
  "userCount": "23",
  "firstSeen": "2026-02-03T14:00:00Z",
  "lastSeen": "2026-02-04T10:30:00Z",
  "level": "error",
  "permalink": "https://employmenthero.sentry.io/issues/12345/"
}
```

#### Step 2: Analyze Stack Trace (CRITICAL)

```bash
# Get latest event with full details (includes stack trace)
sentry api /organizations/$ORG_SLUG/issues/$ISSUE_ID/events/latest/ > event.json

# Extract exception data with stack trace
jq '.entries[] | select(.type == "exception")' event.json

# Or extract just stack frames
jq '.entries[] | select(.type == "exception") | .data.values[0].stacktrace.frames[]' event.json
```

**Full stack overview:**
- **Top frame** (where error occurred) → Primary investigation target
- **Call chain** → Understand execution flow leading to error
- **Framework frames** → Identify if error is in app code vs. library code
- **File paths and line numbers** → Map to codebase locations
- **Function/method names** → Identify affected components

**Analysis checklist:**
- ✅ Is the error in application code or external library?
- ✅ What function/method threw the error?
- ✅ What was the call path leading to the error?
- ✅ Are there any unexpected frames (indicating recursion, wrong code path)?
- ✅ Can we map stack frames to specific files in our codebase?

#### Step 3: Examine Breadcrumbs (User Journey)

```bash
# Extract breadcrumbs from latest event (already fetched in Step 2)
jq '.entries[] | select(.type == "breadcrumbs") | .data.values' event.json

# Or get breadcrumbs from multiple events
sentry api /organizations/$ORG_SLUG/issues/$ISSUE_ID/events/?limit=5 | \
  jq '.[].entries[] | select(.type == "breadcrumbs") | .data.values'
```

**Breadcrumbs reveal:**
- **Navigation history** → Pages/routes visited before error
- **User interactions** → Clicks, form submissions, API calls
- **Console logs** → Debug messages leading to error
- **Network requests** → API calls made before failure
- **State changes** → Redux/store updates, data mutations

**Look for patterns:**
- ✅ What was the user doing immediately before the error?
- ✅ Was there a specific sequence of actions that triggered it?
- ✅ Were there any failed API calls or network issues?
- ✅ Did the error occur after a specific state change?

#### Step 4: Review HTTP Request Details
...
- **Body** → Request payload (if POST/PUT)
...
event.contexts = {
  ...
  app: { app_version: "1.2.3", build: "456" }
}
```

**Analysis:**
- ✅ Does error affect specific environment only?
- ✅ Is it browser/OS specific?
- ✅ Does it correlate with a specific release/deployment?
- ✅ Is it isolated to specific user segments?
- ✅ Are there infrastructure-related tags (server, region)?

#### Step 6: Examine Additional Data

**Custom data** (application-specific):
```typescript
event.extra = {
  user_id: "12345",
  ...
  database_query_time: "1234ms",
  cache_hit: false
}
```

**Look for:**
- ✅ Feature flags that might be related
- ✅ Performance metrics (slow queries, timeouts)
- ✅ Business logic context (user tier, permissions)
- ✅ Integration data (third-party API responses)
- ✅ Custom debug information logged by developers
